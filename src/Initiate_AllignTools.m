...
path2code = 'E:\data\user\yu-ting\repo\BrainAllignTools';
addpath(genpath([path2code]));
cd([path2code,'\allenCCF-master\SHARP-Track']);
path2CCF = [path2code '\allenCCF-master\SHARP-Track']
pata2ref = [path2code '\reference-atlas-files'];
% chooose your image folder with the tif files
path2image = uigetdir('E:\data\user\yu-ting\test_file_BSc');
...
clc;inputSag = input('Are you using sagittal section? press 1 if yes or 0 if no)');
...
elseif myChannel == 2;
    ...
    myChannel == 2;n_rgb = [1 2];name_rgb = 'red + green';
end
...
% Fist section -> Second Section -> Last section 
Process_Histology
